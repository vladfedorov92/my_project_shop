...
export const setAllProducts = async () => {
  let collectionCount = (await getCountFromServer(productCollection)).data()
    .count;
  if (!collectionCount) {
...
export async function getBreadProducts() {
  const ref = doc(productCollection, "bread");
  const temp = await getDoc(ref);
  // return temp.exists() ? temp.data() : {bread: []}
  return temp;
}
